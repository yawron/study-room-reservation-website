...
## 5.1~5.7 登录接口改造（完成于 2026-07-07）

**需求背景**
注册后的用户必须能通过密码验证登录。原 login 只检查邮箱不验密码、不查数据库，本质是假登录。

**做了什么**
重写 login + me route：查用户 → 验密码 → 签发双 Token → Set-Cookie。me 从 mock 改为 Prisma 查询。

**技术点**
- `bcrypt.compare(raw, hash)`：从哈希提取 salt 后对原始密码做相同哈希再比对
- 401 不区分"用户不存在"和"密码错误"：防止攻击者枚举已注册邮箱

**关键决策**
- 保留 `lib/jwt.ts` 不变：JWT 层已在 mock 阶段验证可用，只替换数据源
