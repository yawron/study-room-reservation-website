...
export default function HomePageClient({ initialRooms }: { initialRooms: Room[] }) {
  ...
  return (
    <div className="flex flex-col">
                  ...
                  <div className="relative h-28 xs:h-36 md:h-56 overflow-hidden bg-gray-100">
                    ...
                    <div className="absolute top-2 right-2 md:top-3 md:right-3">
                      <div className="hidden md:block">
                        <Badge variant={room.isAvailable ? 'success' : 'neutral'}>{room.isAvailable ? '空闲' : '满员'}</Badge>
                      </div>
                    </div>
      ...
      {selectedRoom && (
        <BookingModal room={selectedRoom} isOpen={!!selectedRoom} onClose={() => setSelectedRoom(null)} onSuccess={() => setFilter(filter)} />
      )}
    </div>
  );
}
