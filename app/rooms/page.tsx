...
import { Users, Wifi, Filter, ArrowRight, Star, MessageSquare, ChevronDown } from 'lucide-react';
    ...
    <div className="flex flex-col min-h-screen">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-6 md:py-12 w-full">
        ...
        {loading ? (
            ...
            {filteredRooms.map((room) => (
                ...
                <div className="p-3 md:p-5 flex flex-col flex-grow">
                    ...
                    <div className="flex items-center truncate max-w-[60px] md:max-w-none"><Wifi className="w-3 h-3 md:w-4 md:h-4 mr-1"/>WiFi</div>
                    </div>

                    <p className="hidden md:block text-gray-600 text-xs md:text-sm line-clamp-2 mb-4 md:mb-6 flex-grow">{room.description}</p>
        ...
        {selectedFeedbackRoom && (
            <FeedbackModal 
                room={selectedFeedbackRoom}
                isOpen={!!selectedFeedbackRoom}
                onClose={() => setSelectedFeedbackRoom(null)}
            />
        )}
      </div>
    </div>
